...
window.Funnelback.SessionCart = (function() {
  ...
  const CartCount = {
    ...
    init: function(options) {
      ...
      CartCount.element = ElementUtil.create(CartCount.selector, ElementUtil.findOnce(options.cartCount.selector), options.cartCount.elementType, CartCount.template(CartCount.data(0)), {class: options.cartCount.classes, tabindex: options.cartCount.tabIndex, title: CartCount.title(0)});
      
      if (options.cartCount.enableOnClick) ElementUtil.addEvent(CartCount.element, 'click', Constructor.prototype.toggle);
    },
